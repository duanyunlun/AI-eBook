...
触屏阅读默认收起上下悬浮控件，轻点阅读区域切换显示；中间区域左滑打开左侧主菜单，右滑打开右侧伴读栏。纵向滚动、双指缩放、长按选区及批注点击不触发横滑。系统取消触摸时放弃识别，不申请系统手势排除区域；外观设置可调整两侧滑动禁区（24–120 CSS px，默认 32）。
...
运行 `PATH="$PWD/.build-cache/android-sdk/platform-tools:$PATH" node scripts/android-touch-test.mjs`，在已启动的 Android 调试版上验证点击浮层、左滑主菜单、右滑伴读栏、边缘禁区、纵向滚动、长按、双指触摸、系统取消触摸及设置持久化。无开孔模拟器增加 `ANDROID_NO_CUTOUT=1` 检查隐藏状态栏确实增加可用高度。
