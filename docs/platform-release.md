...
阅读区顶部提供“文字重排 / 查看原版”双向切换，移动端点击阅读区即可显示，切换保留原页码。重排依据 PDF 文字行坐标、段前间距、首行缩进、字号变化及短行句尾推断段落；缺少坐标时保留行边界，不再直接合并所有单换行。复杂版式的段落仍可能需要人工核对。

“设置 → 外观 → PDF 阅读”可切换原版、裁白边、文字重排。裁白边按可视宽度放大，左右各裁去 0–20%（默认 10%），由用户调整以避免裁掉页边内容。原版保留 PDF 排印；重排逐页提取文字并按屏幕宽度换行，保留原页码及“查看原版”入口，按需加载附近页面。

“外观 → 字体”区分界面字体与正文字体；正文字体可选系统、衬线、无衬线，字号 16–32px、三档行距，适用于文字书籍与 PDF 重排，独立于 UI 字号。重排沿用阅读配色，包括自定义背景。偏好在本地保存，桌面和移动端均可使用。

批注仍绑定书籍版本和原 PDF 页码；裁边保存原页坐标，重排使用文字位置，切换版式时按选文重新定位。重复且无法唯一匹配的选文不猜测高亮位置，仍可从批注栏查看。重排只显示提取文字，不复刻表格、插图、多栏及脚注排版；扫描页提示返回原版，不包含 OCR，截图操作需切回原版。

`src/pdf-reading-ui.test.html` 使用真实 PDF.js 解析生成的测试 PDF，验证裁边坐标、原版与重排的双向批注定位、字号、空白页提示及返回原页；可在 320px、390px iframe 和桌面宽度运行。`npm test` 检查印刷换行合并和文本提取。
